"""
Input validation for the OODA Terminal API.

Validates query parameters for /ooda/terminal, /ooda/site, and /ooda/data-period
...
def _decode_cursor(cursor: str) -> dict:
    ...
    if "terminal_device_id" not in data:
        raise ValidationError("Cursor is missing required field 'terminal_device_id'")
...
def validate_terminal_params(params: dict) -> dict:
    """
    Validate and normalise query parameters for GET /ooda/terminal.

    Required: terminal_device_id, site_id, start, end
    ...
    """
    # --- Required fields ---
    for field in ("terminal_device_id", "site_id", "start", "end"):
    ...
    # --- Pattern validation ---
    _validate_safe_id(terminal_device_id, "terminal_device_id")
    ...
    if cursor:
        ...
        if cursor_data["terminal_device_id"] != terminal_device_id:
            raise ValidationError(
                f"Cursor 'terminal_device_id' ({cursor_data['terminal_device_id']!r}) does not match "
                f"request 'terminal_device_id' ({terminal_device_id!r})"
            )
...
def validate_site_params(params: dict) -> dict:
    """
    Validate and normalise query parameters for GET /ooda/site.
...
def validate_data_period_params(params: dict) -> dict:
    """Validate params for GET /ooda/data-period."""
    if not params.get("site_id"):
        raise ValidationError("Missing required parameter: 'site_id'")
    site_id = params["site_id"]
    _validate_safe_id(site_id, "site_id")
    result = {"site_id": site_id}
    terminal_device_id = params.get("terminal_device_id")
    if terminal_device_id:
        _validate_safe_id(terminal_device_id, "terminal_device_id")
        result["terminal_device_id"] = terminal_device_id
    return result